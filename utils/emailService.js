...
const createTransporter = () => {
  if (!process.env.EMAIL_USER) {
    console.log(
      "[EmailService] EMAIL_USER not set, using mock transporter for testing",
    );
    return {
  ...
  console.log(
    "[EmailService] Creating real transporter with user:",
...
const sendEmail = async (options) => {
  try {
    ...
    console.log("[EmailService] Email details for debugging:", options);
...
const emailService = {
  sendVerificationEmail: async (email, otp) => {
    try {
      console.log(
        "[EmailService] Sending verification email to:",
        email,
        "OTP:",
        otp,
      );
      ...
      await transporter.sendMail(mailOptions);
      console.log(
        "[EmailService] Verification email sent successfully to:",
        email,
      );
      return true;
    } catch (error) {
      console.error("[EmailService] Error sending verification email:", error);
      console.log("[EmailService] OTP for testing:", otp);
      return true;
    }
  },
  ...
  sendPasswordResetEmail: async (email, resetUrl) => {
    try {
      ...
      await transporter.sendMail(mailOptions);
      console.log(
        "[EmailService] Password reset email sent successfully to:",
        email,
      );
      return true;
    } catch (error) {
      ...
      console.log("[EmailService] Reset URL for testing:", resetUrl);
      return true;
    }
  },
